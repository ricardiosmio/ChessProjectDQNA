...
class ChessGUI:
    ...
    def check_game_status(self):
        if self.board.is_checkmate():
            ...
            self.status_label.config(text="Draw due to insufficient material.")
        elif self.board.is_seventyfive_moves():
            self.status_label.config(text="Draw due to 75-move rule.")
        elif self.board.is_fivefold_repetition():
            self.status_label.config(text="Draw due to fivefold repetition.")
        elif self.board.is_variant_draw():
            self.status_label.config(text="Draw due to variant-specific rule.")
